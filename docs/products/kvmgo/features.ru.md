...
### **Передача текста**
Легко передавайте текст, имитируя нажатия клавиш — идеально для имен пользователей, паролей и фрагментов кода. Поддерживает символы ASCII, включая символы и знаки пунктуации.

- **Хост → Цель**: Отправляйте текст через эмулируемые нажатия клавиш
- **Цель → Хост**: Копируйте текст с экрана цели на хост через OCR (только macOS)

!!! warning "Ограничения передачи текста"
    - **Нет интеграции с буфером обмена**: Эмулирует только ввод текста, без передачи изображений или документов
    - **Только ASCII**: Ограничено символами ASCII (без поддержки китайского, японского, корейского и т. д.)
    - **Учитывайте длину**: Лучше для короткого текста; большие блоки могут вызывать проблемы с производительностью
    - **Функция OCR**: Передача текста Цель → Хост в настоящее время доступна только на macOS

### **Удобство plug-and-play**
На целевом устройстве не требуется установка программного обеспечения. Управление начинается сразу после подключения без оставления следов программного обеспечения.

### **Интеграция аудио**
Встроенный аудио-проход для полного мультимедийного опыта. (Не поддерживается на KVM-Go VGA; подтверждение ожидается для KVM-Go DP.)

### **Возможность Bluetooth**
Встроенная поддержка Bluetooth позволяет использовать нативные функции приложения iPadOS (скоро), делая KVM-GO одним из немногих KVM, которые нативно работают с iPad.
