...
description: 關於 KVM-Go 系列的常見問題，涵蓋功能、兼容性和預發佈信息。 歡迎訪問我們下一代 Openterface KVM-Go 系列的常見問題頁面。 如果您沒有找到所需的答案，請發送郵件至 或加入我們的社區 或 。 ⚠️ 注意：KVM-Go 目前處於預發佈開發階段。隨着我們完善產品，功能、規格和設計可能會有所變化。
...
⚠️ **注意**：KVM-Go 目前處於預發佈開發階段。隨着我們完善產品，功能、規格和設計可能會有所變化。
...
**:material-chat-question:{ .faq } 啓動速度有多快？**

硬件啓動時間不到 1 秒，可以立即進行故障排除，不會延遲或干擾您的工作流程。
...
有——訪問 [Openterface Viewer](https://openterface-viewer.pages.dev/) 獲取無需安裝的基於瀏覽器的應用程序（適用於 Chrome、Edge、Safari）。非常適合快速訪問或無法在主機計算機上安裝軟件的情況。感謝我們出色的社區，特別是啓動此項目的 [@kashalls](https://github.com/kashalls)。
...
還沒有，隨着我們在開發過程中完善產品，功能、規格和設計可能會有所變化。
