...
詳情請參閱[鍵盤和鼠標控制故障排除](/products/minikvm/support/keyboard-mouse-control/)。
...
分步說明請參閱 [診斷自檢指南（macOS）](/products/minikvm/support/diagnostic-self-check/) 或 [診斷自檢指南（Windows）](/products/minikvm/support/diagnostic-self-check-windows/)。

**:material-chat-question:{ .faq } USB 集線器問題**

使用**有源集線器**以避免電壓下降導致不穩定。無源集線器可能導致供電不足和 HID（鍵盤/鼠標）行為異常。詳情請參閱[鍵盤和鼠標控制故障排除](/products/minikvm/support/keyboard-mouse-control/)。
