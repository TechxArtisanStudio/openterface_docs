...
description: 关于 Mini-KVM 的常见问题，涵盖功能、兼容性、故障排除和未来计划。 欢迎来到我们旗舰产品 Openterface Mini-KVM 的常见问题页面。 如果您找不到需要的信息，请发送邮件至 或加入我们的社区 或 。 ⚠️ 常见问题可能已过时 — 如果您发现需要更新的内容，请告知我们。
...
-   对于更长的电缆，通过以下方式注入电源：
    -   USB-A 公对公电缆
    -   [扩展引脚](/products/minikvm/extension-pins/)
    -   USB-C Y 分线器
-   同样的规则适用于**黑色目标电缆**。
...
详情请参阅[键盘和鼠标控制故障排除](/products/minikvm/support/keyboard-mouse-control/)。
...
不直接支持，但[扩展引脚](/products/minikvm/extension-pins/)允许未来的 ATX 控制模块。
...
- **macOS：** [诊断自检指南（macOS）](/products/minikvm/support/diagnostic-self-check/) — 设置 → 高级与调试 → 打开诊断工具
- **Windows：** [诊断自检指南（Windows）](/products/minikvm/support/diagnostic-self-check-windows/) — 高级 → 硬件诊断
...
分步说明请参阅 [诊断自检指南（macOS）](/products/minikvm/support/diagnostic-self-check/) 或 [诊断自检指南（Windows）](/products/minikvm/support/diagnostic-self-check-windows/)。

**:material-chat-question:{ .faq } USB 集线器问题**

使用**有源集线器**以避免电压下降导致不稳定。无源集线器可能导致供电不足和 HID（键盘/鼠标）行为异常。详情请参阅[键盘和鼠标控制故障排除](/products/minikvm/support/keyboard-mouse-control/)。

**:material-chat-question:{ .faq } 应用程序显示无视频或控制无响应**

1. 断开所有电缆，等待几秒，重新连接。
2. 检查[键盘和鼠标控制故障排除](/products/minikvm/support/keyboard-mouse-control/)以了解 HID 问题（电缆、集线器、HID 重置）。
3. 运行[诊断自检](/products/minikvm/support/diagnostic-self-check/)（macOS）或[硬件诊断](/products/minikvm/support/diagnostic-self-check-windows/)（Windows）以验证设备。
...
-   确认最新的主机应用程序
-   运行[诊断自检](/products/minikvm/support/diagnostic-self-check/)以捕获日志
