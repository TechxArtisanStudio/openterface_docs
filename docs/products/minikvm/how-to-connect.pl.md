...
description: "Schritt-dla-Schritt-Przewodnik zur Einrichtung von Openterface Mini-KVM. Lernen Sie, wie Sie Ihren Host-Computer i Ihr Zielgerät z detaillierten Anweisungen dla USB-C-, HDMI- i Peripheriegeräte-Połączenieen verbinden. Enthält Schnittstellenbeschreibungen i wichtige Einrichtungstipps."
keywords: "Mini-KVM Einrichtung, KVM Połączeniesanleitung, USB-C KVM Einrichtung, HDMI KVM Połączenie, KVM Instalacjasanleitung, Computer-Peripheriegeräte Einrichtung, USB-Gerät Połączenie, KVM Schnittstellenanleitung, Headless-Computer Einrichtung, KVM Konfiguration"
---
...
1. **Host-Połączenie** (Orange Seite): Host-Computer z 1,5m Type-C-Kabel verbinden
2. **Ziel-Połączenie** (Schwarze Seite): Zielgerät z 0,3m Type-C-Kabel verbinden
3. **Video-Połączenie** (Schwarze Seite): Video-Ausgabe des Ziels an HDMI-Port anschließen
4. **USB-Gerät** (Optional): Nach Abschluss der Schritte 1-3 an USB-A-Port anschließen

!!! note "Anforderungen" - **Host**: Erfordert Instalacja von [Openterface App](/app) - **Ziel**: Keine App erforderlich - unterstützt Windows, macOS, Linux, Android, iOS - **Video**: Bereitgestelltes HDMI-Kabel lub VGA-to-HDMI-Konverter verwenden
...
- ① **Host USB-C**: Datenotragung zum Host-Computer
...
!!! warning "Wichtige Hinweise" - **Stromversorgung**: USB-Geräte sollten 1,5W Verbrauch nie oschreiten - **USB-A-Verriegelung**: Erfordert zusätzliche Kraft zum Ein-/Ausschieben (kleine Geräte vermeiden) - **USB-Hub**: Nur extern versorgte Hubs verwenden; tiefe USB-Bäume vermeiden - **Umschaltung**: Siehe [USB-Port-Umschaltungsanleitung](../usb-switch) dla Details
