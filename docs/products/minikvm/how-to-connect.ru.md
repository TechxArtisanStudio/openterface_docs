...
description: "Schritt-для-Schritt-Руководство zur Einrichtung von Openterface Mini-KVM. Lernen Sie, wie Sie Ihren Host-Computer и Ihr Zielgerät с detaillierten Anweisungen для USB-C-, HDMI- и Peripheriegeräte-Подключениеen verbinden. Enthält Schnittstellenbeschreibungen и wichtige Einrichtungstipps."
keywords: "Mini-KVM Einrichtung, KVM Подключениеsanleitung, USB-C KVM Einrichtung, HDMI KVM Подключение, KVM Установкаsanleitung, Computer-Peripheriegeräte Einrichtung, USB-Gerät Подключение, KVM Schnittstellenanleitung, Headless-Computer Einrichtung, KVM Konfiguration"
---
...
1. **Host-Подключение** (Orange Seite): Host-Computer с 1,5m Type-C-Kabel verbinden
2. **Ziel-Подключение** (Schwarze Seite): Zielgerät с 0,3m Type-C-Kabel verbinden
3. **Video-Подключение** (Schwarze Seite): Video-Ausgabe des Ziels an HDMI-Port anschließen
4. **USB-Gerät** (Optional): Nach Abschluss der Schritte 1-3 an USB-A-Port anschließen

!!! note "Anforderungen" - **Host**: Erfordert Установка von [Openterface App](/app) - **Ziel**: Keine App erforderlich - unterstützt Windows, macOS, Linux, Android, iOS - **Video**: Bereitgestelltes HDMI-Kabel или VGA-to-HDMI-Konverter verwenden
...
- ① **Host USB-C**: Datenоtragung zum Host-Computer
...
!!! warning "Wichtige Hinweise" - **Stromversorgung**: USB-Geräte sollten 1,5W Verbrauch не оschreiten - **USB-A-Verriegelung**: Erfordert zusätzliche Kraft zum Ein-/Ausschieben (kleine Geräte vermeiden) - **USB-Hub**: Nur extern versorgte Hubs verwenden; tiefe USB-Bäume vermeiden - **Umschaltung**: Siehe [USB-Port-Umschaltungsanleitung](../usb-switch) для Details
