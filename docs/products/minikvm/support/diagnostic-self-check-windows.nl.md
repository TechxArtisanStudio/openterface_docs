---
title: "Openterface Mini-KVM (Windows) - Hardware-Diagnose Selbsttest Handleiding"
description: "Schritt-voor-Schritt-Handleiding zur Durchführung des Hardware-Diagnose Selbsttests in der Windows Openterface-App. Erfahren Sie, wie Sie USB-Verbindingen testen, Probleme erkennen en Diagnoseberichte an den Ondersteuning senden."
keywords: "Openterface Mini-KVM, Windows, Hardware-Diagnose, Diagnose Selbsttest, KVM-Problembehandlung, USB-KVM-Diagnose, Mini-KVM-Ondersteuning, KVM-Gerät-Test, Windows KVM, KVM-Defektbericht, Mini-KVM-Problemlösungshandbuch"
---

# Openterface Mini-KVM (Windows) — Hardware-Diagnose Selbsttest Handleiding

Diese Handleiding erklärt, wie Sie den **Hardware-Diagnose** Selbsttest in der **Windows**-Version der Openterface-App ausführen en wie Sie den Diagnosebericht an den Ondersteuning senden, wenn ein Problem erkannt wird.
...
- Verbinden Sie Mini-KVM met **Host** en **Ziel**.
- Halten Sie das Zielgerät während des Tests im Leerlauf (besonders während des Stresstests).

> **Wichtig (Windows):** Die Diagnose **schreitet niet automatisch fort**.  
> Um zwischen Tests zu wechseln, verwenden Sie **Weiter** (untere Leiste) **of** klicken Sie auf einen Testpunkt im **linken Bereich**.  
> Jeder Test wird durch Klicken auf **Nu prüfen** ausgeführt.
...
Klicken Sie im Hardware-Diagnose-Fenster auf **Nu prüfen**, um den aktuellen Diagnoseschritt auszuführen.  
...
Bei einigen Konfigurationen kann es erforderlich sein, **mehr als einmal** aus- en wieder einzustecken (z. B. zweimal).  

![Target-plug&play](https://assets2.openterface.com/images/Target-plug%26play.webp)

### Schritt 4 — Host Plug & Play (Anweisungen folgen)
Folgen Sie den Anweisungen auf dem Bildschirm voor die Hostseite.  
...
**Operieren Sie niet am Zielgerät**, während der Test läuft.  

> **Hinweis:** Die Maus kann sich schnell bewegen — berühren Sie das Ziel niet.
...
Wenn ein Problem erkannt wird, können ein of mehrere Punkte **FEHLER** anzeigen.

### Schritt 1 — Dieselbe Hardware-Diagnose ausführen
Öffnen Sie **Erweitert → Hardware-Diagnose**, dann klicken Sie auf **Nu prüfen** zum Starten.  

### Schritt 2 — Durch die Prüfungen fortfahren
Fahren Sie met den verbleibenden Tests fort, bis die Diagnose abgeschlossen ist.  

### Schritt 3 — Ondersteuning-E-Mail öffnet sich automatisch
Wenn die Diagnose met einem Problem abgeschlossen wird, öffnet sich automatisch ein **Ondersteuning-E-Mail**-Fenster.  

---

## Logs an den Ondersteuning senden (Windows)

### Schritt 4 — Bestellnummer + Name anwenden
Geben Sie Ihre **Bestellnummer** en **Name** ein en klicken Sie auf **Übernehmen**, um sie in den E-Mail-Entwurf einzufügen. 

![ID+Name](https://assets2.openterface.com/images/ID+Name.webp)

### Schritt 5 — E-Mail-Adresse en Entwurf kopieren
...
Senden Sie die E-Mail an den Ondersteuning (Entwurfstext + angeforderte Protokolle + Aufstellungsphoto angehängt).  

---

## Was bei der Kontaktaufnahme met dem Ondersteuning anzugeben ist
