...
description: "Schritt-için-Schritt-Kılavuz zur Einrichtung von Openterface Mini-KVM. Lernen Sie, wie Sie Ihren Host-Computer ve Ihr Zielgerät ile detaillierten Anweisungen için USB-C-, HDMI- ve Peripheriegeräte-Bağlantıen verbinden. Enthält Schnittstellenbeschreibungen ve wichtige Einrichtungstipps."
keywords: "Mini-KVM Einrichtung, KVM Bağlantısanleitung, USB-C KVM Einrichtung, HDMI KVM Bağlantı, KVM Kurulumsanleitung, Computer-Peripheriegeräte Einrichtung, USB-Gerät Bağlantı, KVM Schnittstellenanleitung, Headless-Computer Einrichtung, KVM Konfiguration"
---
...
1. **Host-Bağlantı** (Orange Seite): Host-Computer ile 1,5m Type-C-Kabel verbinden
2. **Ziel-Bağlantı** (Schwarze Seite): Zielgerät ile 0,3m Type-C-Kabel verbinden
3. **Video-Bağlantı** (Schwarze Seite): Video-Ausgabe des Ziels an HDMI-Port anschließen
4. **USB-Gerät** (Optional): Nach Abschluss der Schritte 1-3 an USB-A-Port anschließen

!!! note "Anforderungen" - **Host**: Erfordert Kurulum von [Openterface App](/app) - **Ziel**: Keine App erforderlich - unterstützt Windows, macOS, Linux, Android, iOS - **Video**: Bereitgestelltes HDMI-Kabel veya VGA-to-HDMI-Konverter verwenden
...
- ① **Host USB-C**: Datenhakkındatragung zum Host-Computer
...
!!! warning "Wichtige Hinweise" - **Stromversorgung**: USB-Geräte sollten 1,5W Verbrauch değil hakkındaschreiten - **USB-A-Verriegelung**: Erfordert zusätzliche Kraft zum Ein-/Ausschieben (kleine Geräte vermeiden) - **USB-Hub**: Nur extern versorgte Hubs verwenden; tiefe USB-Bäume vermeiden - **Umschaltung**: Siehe [USB-Port-Umschaltungsanleitung](../usb-switch) için Details
