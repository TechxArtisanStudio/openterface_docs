...
description: "Openterface Mini-KVM (Windows) - 硬件診斷自檢指南: 本指南介紹如何在 Windows 版 Openterface 應用中運行硬件診斷自檢，以及如何在檢測到問題時將診斷報告發送給支持團隊。 — support/diagnostic-self-check-windows"
...
### 步驟 3 — 目標即插即用（按提示操作）
當**目標即插即用**要求您重新連接目標線纜時，請按照屏幕上的說明操作。  
