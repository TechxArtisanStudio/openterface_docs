...
詳情請參閲[鍵盤和鼠標控制故障排除](/product/minikvm/support/keyboard-mouse-control/)。
...
分步説明請參閲 [診斷自檢指南（macOS）](/product/minikvm/support/diagnostic-self-check/) 或 [診斷自檢指南（Windows）](/product/minikvm/support/diagnostic-self-check-windows/)。

**:material-chat-question:{ .faq } USB 集線器問題**

使用**有源集線器**以避免電壓下降導致不穩定。無源集線器可能導致供電不足和 HID（鍵盤/鼠標）行為異常。詳情請參閲[鍵盤和鼠標控制故障排除](/product/minikvm/support/keyboard-mouse-control/)。
