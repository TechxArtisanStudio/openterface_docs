...
| 權限被拒絕 | 將用戶加入 `dialout` 和 `video` 組（Linux） |
...
如需要，從 [WCH CH34xDriver GitHub](https://github.com/WCHSoftGroup/ch34xser_macos) 安裝 WCH CH34x 驅動。在 **System Settings > General > Login Items & Extensions > Driver Extensions** 中啟用。
...
1. **啟用硬件加速** — Preferences > Video > Hardware Acceleration（VAAPI、V4L2-M2M）
...
查看狀態欄中的 FPS 計數器。若實際 FPS 低於目標，說明管道存在瓶頸。在 FFmpeg 幀處理器中啟用丟幀以優先保證流暢播放。
...
6. **檢查 Openterface 應用藍牙權限** — 若首次啟動被拒絕，請在設置中重新啟用
