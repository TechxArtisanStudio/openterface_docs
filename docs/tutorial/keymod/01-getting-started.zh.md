---
title: "KeyMod 教程 - 入门"
description: 安装 KeyCmd 应用、连接 KeyMod 设备，并在 5 分钟内发送第一个按键。 安装 KeyCmd 应用、连接 KeyMod 硬件，并在 5 分钟内发送第一个按键。
keywords: "KeyMod 入门, KeyMod 设置, KeyMod 安装, 连接 KeyMod"
---

# 1. 入门

安装 KeyCmd 应用、连接 KeyMod 硬件，并在 5 分钟内发送第一个按键。

## 您需要准备

- **Openterface KeyMod 硬件** — 已开机且在范围内
- **手机或平板** — 已安装 KeyCmd 应用的 Android 设备
...
1. 打开手机浏览器，访问 [KeyCmd beta APK (v0.19)](https://assets2.openterface.com/data/KeyCmd-release-0.19.apk)
2. 点击下载的文件进行安装
3. 若 Android 提示，请为浏览器允许 **「安装未知应用」**
...
KeyCmd 可通过两种方式连接 Openterface KeyMod 硬件：
...
> **提示：** 在连接对话框中启用 **「启动时自动连接」**，每次打开应用时 KeyCmd 会自动重连。应用会记住上次连接类型（USB 或 BLE）。
