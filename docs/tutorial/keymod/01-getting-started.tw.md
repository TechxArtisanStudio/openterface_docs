---
title: "KeyMod 教程 - 入門"
description: "KeyMod 教程 - 入門: 安裝 KeyMod 應用、連接 KeyMod 硬件，並在 5 分鐘內發送第一個按鍵。. 官方 Openterface 文檔：安裝步驟、常見問題與故障排查，面向 IT 與開發者。 — keymod/01-getting-started"
keywords: "KeyMod 入門, KeyMod 設置, KeyMod 安裝, 連接 KeyMod"
---

# 1. 入門

安裝 KeyMod 應用、連接 KeyMod 硬件，並在 5 分鐘內發送第一個按鍵。

## 您需要準備

- **Openterface KeyMod 硬件** — 已開機且在範圍內
- **手機或平板** — 已安裝 KeyMod 應用的 Android 設備
...
1. 打開手機瀏覽器，訪問 [KeyMod GitHub Releases 頁面](https://github.com/TechxArtisanStudio/Openterface_KeyMod_Android/tags)
2. 下載最新的 `.apk` 文件
3. 點擊下載的文件進行安裝
4. 若 Android 提示，請為瀏覽器允許 **「安裝未知應用」**

也可從源碼構建——見下方 [從源碼構建](#從源碼構建)。
...
> **提示：** 在連接對話框中啟用 **「啟動時自動連接」**，每次打開應用時 KeyMod 會自動重連。應用會記住上次連接類型（USB 或 BLE）。

## 步驟 3：選擇模式 — 歡迎與指南

啟動後會看到 **歡迎與指南** 屏幕及模式卡片：
...
**「記住我的選擇」** — 勾選後，下次啟動將跳過歡迎屏幕，直接進入上次使用的模式。
