...
description: "解決常見 KeyMod 問題：連接、按鍵無響應、藍牙配對失敗、語音輸入錯誤等。"
...
| **USB 權限被拒絕** | 進入 Android 設置 → 應用 → KeyMod → 權限 → 啟用 USB。重新插線。 |
...
在連接對話框中啟用 **「Auto-connect on startup」**。KeyMod 會記住上次連接類型（USB 或 BLE）及上次配對的 BLE 設備。

### USB 插拔檢測

KeyMod 監聽 Android USB 插拔廣播。拔出 USB 線時連接狀態立即更新。若啟用自動連接，重新插入會嘗試重連。
...
| 現象 | 解決方案 |
|---|---|
| **觸控板無響應** | 在設置中啟用觸覺反饋。嘗試觸控板幫助疊加層 (?) 確認手勢支持。 |
...
| 現象 | 解決方案 |
|---|---|
| **未說話時仍在錄音** | 檢查「靜音時自動暫停」。減少背景噪音。清晰、靠近麥克風說話。 |
